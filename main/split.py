...
'''
    ...
    def pardakht(self,who_pay,who_recieve,amount):
        self.who_pay = who_pay
        self.who_recieve = who_recieve
        self.amount = amount

        who_pay_money = self.cur.execute('SELECT money FROM split_table WHERE name = ? ',(self.who_pay,)).fetchone()
        who_recieve_money = self.cur.execute('SELECT money FROM split_table WHERE name = ? ',(self.who_recieve,)).fetchone()
        
        self.cur.execute('UPDATE split_table SET money = ? WHERE name = ? ',((who_pay_money[0]+self.amount),self.who_pay))
        self.cur.execute('UPDATE split_table SET money = ? WHERE name = ? ',((who_recieve_money[0]-self.amount),self.who_recieve))
